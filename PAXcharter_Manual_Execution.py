...
try:
    with mydb1.cursor() as cursor:
        sql = "SELECT * FROM paxminer.regions where region = 'MeCa' and firstf_channel IS NOT NULL AND send_firstf_messages = 1" # <-- Update this for whatever region is being tested
        cursor.execute(sql)
...
for index, row in regions_df.iterrows():
    ...
    #firstf = 'U40HBU8BB' # <--- Use this if sending a test msg to a specific user
